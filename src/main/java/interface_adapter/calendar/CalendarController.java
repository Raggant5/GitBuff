...
public class CalendarController {
    ...
    /**
     * Loads the current user's calendar events.
     */
    ...
    public void synchronizeMeals(List<MealDisplayData> meals) {
        if (calendarViewModel.getState().getErrorMessage() == null) {
            final List<MealDisplayData> desiredMeals = collectValidMeals(meals);
            removeOutdatedMealEvents(desiredMeals);
    ...
    public void replaceWorkoutPlans(List<WorkoutPlanDisplayData> workoutPlans) {
        if (calendarViewModel.getState().getErrorMessage() == null) {
            final List<ScheduledWorkout> desiredWorkouts = collectScheduledWorkouts(workoutPlans);
            removeOutdatedWorkoutEvents(desiredWorkouts);
    ...
    private List<MealDisplayData> collectValidMeals(List<MealDisplayData> meals) {
        final List<MealDisplayData> validMeals = new ArrayList<>();
        if (meals != null) {
    ...
    private void removeOutdatedMealEvents(List<MealDisplayData> desiredMeals) {
    ...
    private List<ScheduledWorkout> collectScheduledWorkouts(List<WorkoutPlanDisplayData> workoutPlans) {
        final List<ScheduledWorkout> scheduledWorkouts = new ArrayList<>();
        if (workoutPlans != null) {
            for (WorkoutPlanDisplayData plan : workoutPlans) {
                if (plan != null && plan.getExercises() != null
                    ...
                    if (date != null) {
                        scheduledWorkouts.add(new ScheduledWorkout(
    ...
    private void removeOutdatedWorkoutEvents(List<ScheduledWorkout> desiredWorkouts) {
        final List<CalendarEventDisplayData> currentEvents =
                List.copyOf(calendarViewModel.getState().getCalendarEvents());
        for (CalendarEventDisplayData event : currentEvents) {
            if (event.getDescription() != null
    ...
    private ScheduledWorkout findMatchingWorkout(
        ...
        ScheduledWorkout matchingWorkout = null;
        for (ScheduledWorkout workout : workouts) {
            if (matchingWorkout == null
                    && workout.title().equals(event.getTitle())
    ...
    private MealDisplayData findMatchingMeal(List<MealDisplayData> meals, CalendarEventDisplayData event) {
        MealDisplayData matchingMeal = null;
        for (MealDisplayData meal : meals) {
            if (matchingMeal == null
                    && ("Meal: " + meal.getName()).equals(event.getTitle())
    ...
    private CalendarEventDisplayData findEventWithDescription(String description) {
        CalendarEventDisplayData matchingEvent = null;
        for (CalendarEventDisplayData event : calendarViewModel.getState().getCalendarEvents()) {
            if (matchingEvent == null && description.equals(event.getDescription())) {
                matchingEvent = event;
            }
        }
    ...
    private LocalDate parseWorkoutDate(String dateText) {
        ...
        if (dateText != null && !dateText.isBlank()) {
            ...
            try {
                ...
                if (parsedDate.isBefore(today.minusDays(1))) {
